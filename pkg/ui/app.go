...
func (a *App) Init() tea.Cmd {
	// Start with context selection workflow
	a.state.CurrentView = types.ContextView

	// Load available contexts
	if err := a.contextSelector.LoadContexts(); err != nil {
		// If context loading fails, show resource view with error
	...
	// Setup tick timer
	return tea.Tick(a.config.Settings.RefreshInterval, func(t time.Time) tea.Msg {
		return TickMsg(t)
	})
}
...
func (a *App) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case memoryMsg:
		a.state.MemoryOutput = msg.output
		return a, nil

	case logsMsg:
		a.state.LogsOutput = msg.output
		return a, nil

	case topMsg:
		a.state.TopOutput = msg.output
		return a, nil

	case FeedbackSubmittedMsg:
		a.state.FeedbackSubmitting = false
		if msg.success {
			// Show thank you message and set status
			a.state.StatusMessage = "Thanks for your feedback!"
			a.state.ShowFeedbackForm = false
			a.state.CurrentView = types.ResourceView
			// Wait a moment to show the message, then quit
			return a, tea.Sequence(tea.Tick(2*time.Second, func(t time.Time) tea.Msg {
				return tea.Quit()
			}))
		} else {
...
func (a *App) renderMainView() string {
	...
	// No frame - just return the plain content
...
func (a *App) renderDescribeView() string {
	...
	resourceInfo := fmt.Sprintf("Resource: %s | Type: %s",
...
func (a *App) renderTopView() string {
	var sections []string
	...
	// Context info at top
	contextInfo := fmt.Sprintf("Context: %s | Namespace: %s", a.state.Context, a.state.Namespace)
	contextInfoStyle := lipgloss.NewStyle().
		Foreground(lipgloss.Color("245")).
		Padding(0, 1)
	sections = append(sections, contextInfoStyle.Render(contextInfo))

	// Title
	title := "Kubernetes Top Resources (Nodes & Pods)"
	titleStyle := lipgloss.NewStyle().
		Bold(true).
		Foreground(lipgloss.Color("39")).
		Background(lipgloss.Color("236")).
	...
	// Get scrollable content
	content := a.getScrollableTopContent()
...
func (a *App) getScrollableTopContent() string {
	if a.state.TopOutput == "" {
		return "Loading top resources..."
	}

	lines := strings.Split(a.state.TopOutput, "\n")
	viewHeight := a.getTopViewHeight() - 2 // Account for padding

	// Calculate visible range
	startLine := a.state.TopScrollOffset
	endLine := startLine + viewHeight
	...
	// Apply selection highlighting if active
	if a.state.SelectionActive && a.state.CurrentView == types.TopView {
		content = a.renderWithSelectionHighlight(content, 3) // viewStartY = 3
	}
...
func (a *App) getTopScrollInfo() string {
	if a.state.TopOutput == "" {
		return "Loading..."
	}
	...
	lines := strings.Split(a.state.TopOutput, "\n")
	totalLines := len(lines)
	viewHeight := a.getTopViewHeight() - 2
	currentLine := a.state.TopScrollOffset + 1
...
func (a *App) fetchResourceDescription() tea.Msg {
	...
	// Execute describe command using detected tool
	cmd := exec.Command(a.state.Tool, "describe", resourceType, a.state.SelectedResource.Name, "-n", a.state.Namespace)
...
func (a *App) fetchResourceTags() tea.Msg {
	...
	// Execute command to get ImageStream JSON using detected tool
	cmd := exec.Command(a.state.Tool, "get", "is", a.state.SelectedResource.Name, "-n", a.state.Namespace, "-o", "json")
	output, err := cmd.Output()
...
func (a *App) fetchResourceRequests() tea.Msg {
	...
	if a.state.ResourceType == "pods" {
		...
		cmd = exec.Command(a.state.Tool, "get", "pod", a.state.SelectedResource.Name, "-n", a.state.Namespace, "-o", "jsonpath={range .spec.containers[*]}{.name}{\"\\t\"}{.resources.requests.cpu}{\"\\t\"}{.resources.requests.memory}{\"\\t\"}{.resources.limits.cpu}{\"\\t\"}{.resources.limits.memory}{\"\\n\"}{end}")
	} else {
		// Get container resource requests and limits for a deployment
		cmd = exec.Command(a.state.Tool, "get", "deployment", a.state.SelectedResource.Name, "-n", a.state.Namespace, "-o", "jsonpath={range .spec.template.spec.containers[*]}{.name}{\"\\t\"}{.resources.requests.cpu}{\"\\t\"}{.resources.requests.memory}{\"\\t\"}{.resources.limits.cpu}{\"\\t\"}{.resources.limits.memory}{\"\\n\"}{end}")
	}
		...
		if screenY >= startY && screenY <= endY {
			...
			if startY == endY && startY == screenY {
				...
				if startX < len(line) && endX > startX {
					...
					// Use a bright distinctive selection background that works with any text color
...
func (a *App) fetchResourceYaml() tea.Msg {
	...
	// Execute get command with YAML output using detected tool
	cmd := exec.Command(a.state.Tool, "get", resourceType, a.state.SelectedResource.Name, "-n", a.state.Namespace, "-o", "yaml")
	output, err := cmd.Output()
...
func (a *App) fetchResourceEvents() tea.Msg {
	...
	cmd := exec.Command(a.state.Tool, "get", "events", "-n", a.state.Namespace,
...
func (a *App) fetchResourceDiagram() tea.Msg {
	...
	cmd := exec.Command(a.state.Tool, "get", "deployment", a.state.SelectedResource.Name, "-n", a.state.Namespace, "-o", "yaml")
...
func (a *App) fetchPodMemory() tea.Msg {
	// Only get memory for pods
	if a.state.ResourceType != "pods" {
		return memoryMsg{output: "Memory information is only available for pods"}
	}

	// Get memory usage using kubectl top
	cmd := exec.Command(a.state.Tool, "top", "pod", a.state.SelectedResource.Name, "-n", a.state.Namespace)
	output, err := cmd.Output()
	if err != nil {
		// Fallback to resource requests/limits if top is not available
		cmd = exec.Command(a.state.Tool, "get", "pod", a.state.SelectedResource.Name, "-n", a.state.Namespace, "-o", "jsonpath={range .spec.containers[*]}{.name}{\"\\t\"}{.resources.requests.memory}{\"\\t\"}{.resources.limits.memory}{\"\\n\"}{end}")
		fallbackOutput, fallbackErr := cmd.Output()
		if fallbackErr != nil {
			return memoryMsg{output: fmt.Sprintf("Error getting memory information: %s", err.Error())}
		}

		result := "Memory Requests/Limits (kubectl top not available):\n\n"
		result += "CONTAINER\tREQUESTS\tLIMITS\n"
		result += string(fallbackOutput)
		return memoryMsg{output: result}
	}

	result := "Current Memory Usage:\n\n"
	result += string(output)
	return memoryMsg{output: result}
}

func (a *App) fetchPodLogs() tea.Msg {
	// Only get logs for pods
	if a.state.ResourceType != "pods" {
		return logsMsg{output: "Logs are only available for pods"}
	}

	// Ensure we have a valid tool - fallback to detected tool if needed
	tool := a.state.Tool
	if tool == "" {
		tool = a.ocCommands.GetTool()
		a.state.Tool = tool // Update the state with the detected tool
	}
	...
	if a.state.SelectedResource.Name == "" {
		return logsMsg{output: "No pod selected for logs"}
	}

	// Get pod logs with last 100 lines
	cmd := exec.Command(tool, "logs", a.state.SelectedResource.Name, "-n", namespace, "--tail=100")
	output, err := cmd.Output()
	if err != nil {
		return logsMsg{output: fmt.Sprintf("Error getting logs for pod '%s' in namespace '%s': %s\n\nTip: Make sure the pod exists and is running.", a.state.SelectedResource.Name, namespace, err.Error())}
	}

	if len(output) == 0 {
		return logsMsg{output: fmt.Sprintf("No logs available for pod '%s' in namespace '%s'", a.state.SelectedResource.Name, namespace)}
	}

	return logsMsg{output: string(output)}
}

func (a *App) fetchTopCommand() tea.Msg {
	// Execute the combined top command: kubectl top nodes && echo && kubectl top pods -A
	tool := a.state.Tool
	if tool == "" {
		tool = "kubectl" // fallback to kubectl
	}

	// Build the command: kubectl top nodes && echo && kubectl top pods -A
	cmd := exec.Command("bash", "-c", fmt.Sprintf("%s top nodes && echo && %s top pods -A", tool, tool))
	output, err := cmd.Output()

	if err != nil {
		return topMsg{output: fmt.Sprintf("Error running top command: %s\n\nNote: The 'kubectl top' command requires the metrics-server to be installed in your cluster.", err.Error())}
	}

	if len(output) == 0 {
		return topMsg{output: "No top information available"}
	}

	return topMsg{output: string(output)}
}
...
func (a *App) renderMemoryView() string {
	var sections []string
	...
	// Title
	title := fmt.Sprintf("Memory Usage: %s", a.state.SelectedResource.Name)
	...
	// Get scrollable content
	content := a.getScrollableMemoryContent()
	...
	// Instructions with scroll info
	scrollInfo := fmt.Sprintf("Line %d-%d of %d",
		a.state.MemoryScrollOffset+1,
		min(a.state.MemoryScrollOffset+a.getMemoryViewHeight(), a.getMemoryLineCount()),
		a.getMemoryLineCount())
...
func (a *App) renderLogsView() string {
	...
	// Title
	title := fmt.Sprintf("Pod Logs: %s", a.state.SelectedResource.Name)
	...
	// Get scrollable content
	content := a.getScrollableLogsContent()
	...
	// Instructions with scroll info
	scrollInfo := fmt.Sprintf("Line %d-%d of %d",
		a.state.LogsScrollOffset+1,
		min(a.state.LogsScrollOffset+a.getLogsViewHeight(), a.getLogsLineCount()),
		a.getLogsLineCount())
...
func (a *App) getScrollableMemoryContent() string {
	if a.state.MemoryOutput == "" {
		return "Loading memory information..."
	}

	lines := strings.Split(a.state.MemoryOutput, "\n")
	viewHeight := a.getMemoryViewHeight() - 2 // Account for padding

	// Calculate visible range
	startLine := a.state.MemoryScrollOffset
	endLine := startLine + viewHeight
...
func (a *App) getMemoryViewHeight() int {
	return a.height - 10 // Same as the content area height
}

func (a *App) getMemoryLineCount() int {
	if a.state.MemoryOutput == "" {
		return 0
	}
	return len(strings.Split(a.state.MemoryOutput, "\n"))
}

func (a *App) getScrollableLogsContent() string {
	if a.state.LogsOutput == "" {
		return "Loading logs..."
	}

	lines := strings.Split(a.state.LogsOutput, "\n")
	viewHeight := a.getLogsViewHeight() - 2 // Account for padding

	// Calculate visible range
	startLine := a.state.LogsScrollOffset
	endLine := startLine + viewHeight
...
func (a *App) getLogsViewHeight() int {
	return a.height - 10 // Same as the content area height
}

func (a *App) getLogsLineCount() int {
	if a.state.LogsOutput == "" {
		return 0
	}
	return len(strings.Split(a.state.LogsOutput, "\n"))
}
