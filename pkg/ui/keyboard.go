...
func (a *App) handleKeyPress(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "t":
		return a.handleShowTags()

	case "m":
		return a.handleShowResources()
...
func (a *App) handleRightClick(msg tea.MouseMsg) (tea.Model, tea.Cmd) {
	...
	if a.state.SelectedText != "" {
		// Copy selected text
		a.copyToClipboard(a.state.SelectedText)
	} else {
		// Copy current content (same as Ctrl+Shift+C)
		return a.handleCopyContent()
	}
	return a, nil
}
...
func (a *App) handleMouseRelease(msg tea.MouseMsg) (tea.Model, tea.Cmd) {
	if a.state.SelectionActive {
		...
		// Keep selection active if text was selected
		if a.state.SelectedText == "" {
...
func (a *App) handleBackNavigation() (tea.Model, tea.Cmd) {
	...
	if a.state.PreviousView != 0 {
		...
		if previousView == types.ResourceView && a.state.PreviousResource != "" {
...
func (a *App) handleContextViewKeys(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		...
		// Update current context and switch back to resource view
		a.state.Context = a.contextSelector.GetSelectedContext()
		// Also update the current namespace after context switch
		if currentNs, err := a.ocCommands.GetCurrentNamespace(); err == nil {
...
func (a *App) handleCopyContent() (tea.Model, tea.Cmd) {
	...
	case types.ResourceView:
		...
		if len(a.state.Resources) > 0 && a.state.SelectedIndex < len(a.state.Resources) {
			content = a.state.Resources[a.state.SelectedIndex].Name
		}
...
func (a *App) copyToClipboard(text string) {
	...
	// Use the clipboard library for better cross-platform support
	err := clipboard.WriteAll(text)
	if err != nil {
		// Fallback to native OS commands if clipboard library fails
		a.copyToClipboardFallback(text)
	}
}

// Fallback clipboard implementation using native OS commands
func (a *App) copyToClipboardFallback(text string) {
	var cmd *exec.Cmd
	...
	if cmd != nil {
		cmd.Run() // Ignore errors in fallback
	}
}
...
func (a *App) extractSelectedText() {
	...
	a.state.SelectedText = strings.TrimSpace(content)
}
