...
func (a *App) renderResourceTable() string {
	...
	// Render simple resource list without borders
	header := a.renderSimpleTableHeader()
	rows := a.renderSimpleTableRows(paginatedResources.Items)
	...
	// No borders - just join header and rows
...
func (a *App) renderSimpleTableHeader() string {
	var headers []string
	...
	// Simple header row without borders
	headerRow := strings.Join(headerCells, " ")

	// Add a separator line under header
	separator := strings.Repeat("-", len(headerRow))
	separatorStyled := lipgloss.NewStyle().
		Foreground(lipgloss.Color("240")).
		Render(separator)

	return lipgloss.JoinVertical(lipgloss.Left, headerRow, separatorStyled)
}
...
func (a *App) renderSimpleTableRows(resources []types.Resource) string {
	if len(resources) == 0 {
	...
	for i, resource := range resources {
		isSelected := i == a.getSelectedIndex()
		row := a.renderSimpleTableRow(resource, columnWidths, isSelected, i%2 == 0)
		rows = append(rows, row)
	}

	// Don't add empty rows for borderless display
	return strings.Join(rows, "\n")
}
...
func (a *App) renderMessageRow(widths []int, message string) string {
...
func (a *App) renderSimpleTableRow(resource types.Resource, widths []int, isSelected bool, isEvenRow bool) string {
	age := formatDuration(resource.Age)
	...
	for i, cell := range cells {
		...
		if isSelected {
			...
			// Apply status-based text colors
			switch resource.Status {
			case "Running", "Ready", "Active", "Bound":
				style = style.Foreground(lipgloss.Color("82")) // Bright green
			case "Pending", "Partial":
			...
			case "Error", "Failed", "Not Ready":
				style = style.Foreground(lipgloss.Color("196")) // Red
			default:
				style = style.Foreground(lipgloss.Color("255")) // White text
			}
		}
	...
	// Simple row without borders
	return fmt.Sprintf("%s %s", prefix, strings.Join(formattedCells, " "))
}

func (a *App) renderSimpleEmptyRow(widths []int) string {
	var cells []string
	for _, width := range widths {
		cell := lipgloss.NewStyle().
			Width(width).
...
func (a *App) renderEmptyTable() string {
	// Simple message when no resources
