...
func (a *App) renderStatusBar() string {
	...
	// Include status message if available (don't clear it immediately)
	if a.state.StatusMessage != "" {
		status = a.state.StatusMessage
	}
	...
	// Show text selection info if active
	selectionInfo := ""
	if a.state.SelectionActive {
		if a.state.SelectedText != "" {
			selectionInfo = fmt.Sprintf("│ Text Selected: %d chars", len(a.state.SelectedText))
		} else {
			selectionInfo = "│ Selecting..."
		}
	}

	statusLine := fmt.Sprintf("Status: %s │ Total: %d │ Displayed: %d │ %s %s",
		status,
		a.state.TotalItems,
